...
from reportlab.lib.utils import ImageReader # QR render ke liye zaroori hai
import datetime
import io
import pytz 
import qrcode 
...
if st.button("Generate Premium PDF Quotation"):
    if not cust_name or not veh_name or price is None:
        ...
        # --- QR CODE GENERATION (Google Maps Link) ---
        ...
        # --- DRAW QR CODE (Right side of details) ---
        qr_reader = ImageReader(qr_img_buffer)
        c.drawImage(qr_reader, 465, 615, width=70, height=70)
        c.setFont("Helvetica-Bold", 8)
        c.drawRightString(535, 605, "Scan for Shop Location")
        ...
        for label, val in data:
            ...
            c.drawRightString(450, y, val) # Adjust right margin for QR space
            y -= 25
        ...
        c.save()
        st.success(f"Professional PDF Generated with Shop Location QR!")
